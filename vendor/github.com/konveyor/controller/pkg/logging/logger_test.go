...
func TestLogger(t *testing.T) {
	...
	g.Expect(len(f.entry[3].kvpair)).To(gomega.Equal(2))
	g.Expect(f.entry[3].kvpair[0]).To(gomega.Equal(Stack))
	...
	g.Expect(len(f.entry[4].kvpair)).To(gomega.Equal(2))
	g.Expect(f.entry[4].kvpair[0]).To(gomega.Equal(Stack))
